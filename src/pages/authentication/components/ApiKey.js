...
export const ApiKeyModal = ({
  className, values, handleChange, handleBlur, touched, isValid, handleSubmit, isSubmitting, errors, status
}) => (
  <Modal size='sm' contentClass={className} showModal>
    ...
    <form onSubmit={handleSubmit}>
      <Input
        ...
        icon={<KeyIcon />} />
      <a href='https://account.arena.net/applications' target='_blank' rel='noopener noreferrer'>Get an API Key</a>
      <Button type='submit' onClick={handleSubmit} loading={isSubmitting} disabled={!isValid}>Submit</Button>
    </form>
  </Modal>
)
...
const ApiKeyModalEnhancer = compose(
  ...
  withFormik({
    ...
    handleSubmit: ({ apiKey }, { props, setSubmitting, setStatus }) => {
      setStatus(null)
      props.authenticate({ apiKey, token: props.authUser.token })
        .then(res => {
          setSubmitting(false)
          if (res === 403) {
